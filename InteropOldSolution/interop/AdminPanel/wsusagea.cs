...
// Type: interop.AdminPanel.wsusagea
...
{
  public class wsusagea : UserControl
  {
    ...
    {
      HelpClassWebServices classWebServices = (HelpClassWebServices) this.Session["AdminSelectedWS"];
      if (classWebServices == null)
        return;
      if (classWebServices.ID == 1L)
        this.Session["HelpMenu1"] = (object) "Help/superadmin/HelpServisiImotenListPrebaruvanje.aspx";
      else if (classWebServices.ID == 16L)
        this.Session["HelpMenu1"] = (object) "Help/superadmin/HelpServisiPodatociZaParceliPrebaruvanje.aspx";
      else if (classWebServices.ID == 2L)
        this.Session["HelpMenu1"] = (object) "Help/superadmin/HelpServisiDrzavjanstvoPrebaruvanje.aspx";
      else if (classWebServices.ID == 10L)
        this.Session["HelpMenu1"] = (object) "Help/superadmin/HelpServisiMBAKNPrebaruvanje.aspx";
      else if (classWebServices.ID == 15L)
        this.Session["HelpMenu1"] = (object) "Help/superadmin/HelpServisiMBCUPrebaruvanje.aspx";
      ...
      List<PARAM> paramsForWebservice = new ParamsDAL().GetParamsForWebservice(new WebservicesDAL().GetByID(classWebServices.ID));
      ...
      foreach (PARAM obj in paramsForWebservice)
      {
        ...
        {
          label1.Text = "Даночен број или назив на субјектот";
          textBox.MaxLength = 13;
          label1.Width = (Unit) 300;
          label1.Style.Add("padding-bottom", "10px");
          textBox.Style.Add("Width", "305px");
        ...
        this.ControlParams.Controls.Add((Control) literalControl5);
        ...
        try
        {
          if (obj.Tittle == "Град")
          {
            opstini gradoviDataSet = new WCFServiceAKNDataSetsClient().GetGradoviDataSet();
    ...
    {
      HelpClassWebServices classWebServices = (HelpClassWebServices) this.Session["AdminSelectedWS"];
      this.Session["IsMBS"] = (object) false;
      ...
      {
        this.Session["NaziviCR_CU"] = (object) false;
        this.MultiView.Visible = true;
        this.MultiView.SetActiveView(this.ViewUJP_NAZIV);
        if (this.DropDownListOsnov.SelectedValue != "0" || !this.DropDownListOsnov.Visible)
        {
    ...
    {
      List<PARAM> list = (List<PARAM>) this.Session["WebServiceParametri"];
      if (list == null)
        return;
      int num = 0;
      ...
      while (num <= 3 && !this.Completed)
      {
        ...
        try
        {
          ...
          else
          {
            ...
            DataColumn column1 = new DataColumn("Адреса");
            DataColumn column2 = new DataColumn("Банка");
            DataColumn column3 = new DataColumn("ЕДБ");
            DataColumn column4 = new DataColumn("Факс");
            DataColumn column5 = new DataColumn("Матичен Број");
            DataColumn column6 = new DataColumn("Место");
            DataColumn column7 = new DataColumn("Назив");
            DataColumn column8 = new DataColumn("Тип");
            DataColumn column9 = new DataColumn("Опис");
            DataColumn column10 = new DataColumn("Телефон");
            DataColumn column11 = new DataColumn("Жиро Сметка");
            ...
            row["Адреса"] = (object) dataByEdb.GetPodatociByEDBResult.Telo.DokOsnovniPodatoci.Adresa;
            row["Банка"] = (object) dataByEdb.GetPodatociByEDBResult.Telo.DokOsnovniPodatoci.Banka;
            row["ЕДБ"] = (object) dataByEdb.GetPodatociByEDBResult.Telo.DokOsnovniPodatoci.Edb;
            row["Факс"] = (object) dataByEdb.GetPodatociByEDBResult.Telo.DokOsnovniPodatoci.Faks;
            row["Матичен Број"] = (object) dataByEdb.GetPodatociByEDBResult.Telo.DokOsnovniPodatoci.MB;
            row["Место"] = (object) dataByEdb.GetPodatociByEDBResult.Telo.DokOsnovniPodatoci.Mesto_naziv;
            row["Назив"] = (object) dataByEdb.GetPodatociByEDBResult.Telo.DokOsnovniPodatoci.Naziv;
            row["Тип"] = (object) dataByEdb.GetPodatociByEDBResult.Telo.DokOsnovniPodatoci.Of_opis;
            row["Опис"] = (object) dataByEdb.GetPodatociByEDBResult.Telo.DokOsnovniPodatoci.Opis_nace;
            row["Телефон"] = (object) dataByEdb.GetPodatociByEDBResult.Telo.DokOsnovniPodatoci.Telefon;
            row["Жиро Сметка"] = (object) dataByEdb.GetPodatociByEDBResult.Telo.DokOsnovniPodatoci.Ziro;
    ...
    private void GetFirmiByNaziv()
    {
      ...
      reqrespPortClient.ClientCredentials.ClientCertificate.SetCertificate(StoreLocation.CurrentUser, StoreName.My, X509FindType.FindBySubjectName, "UserUJP");
    ...
    protected void ImageButtonCR_CU_Click(object sender, ImageClickEventArgs e)
    {
      ...
      list1.Add((GridView) this.Session["GridViewList2"]);
      List<GridView> list2 = new List<GridView>();
      list2.Add((GridView) this.Session["GridViewList3"]);
      ComputedTransaction transaction = new InteropDAL().GetTransaction(((USER) this.Session["user"]).username, this.Session["NazivServis"].ToString());
      this.Session["Transactions"] = (object) transaction;
      pdfReport report = new pdfReport();
      report.User = (USER) this.Session["user"];
      ...
      new InteropDAL().InsertToReportAndLog((USER) this.Session["user"], transaction.RecordID.ToString(), report, creationTime);
    ...
    protected void ImageButtonUJP_EDB_Click(object sender, ImageClickEventArgs e)
    {
      try
      {
        ...
        ComputedTransaction transaction = new InteropDAL().GetTransaction(((USER) this.Session["user"]).username, "Единствен Даночен Број На Субјектот");
        this.Session["Transactions"] = (object) transaction;
        pdfReport report = new pdfReport();
    ...
    protected void ImageButtonAKN_Click(object sender, ImageClickEventArgs e)
    {
      ...
      if (!File.Exists(report.PdfFilePath))
        return;
      DateTime creationTime = File.GetCreationTime(report.PdfFilePath);
      this.Session["ReportIme"] = (object) report.PdfFileName;
      this.Session["PdfFilePath"] = (object) report.PdfFilePath;
      new InteropDAL().InsertToReportAndLog((USER) this.Session["user"], transaction.RecordID.ToString(), report, creationTime);
