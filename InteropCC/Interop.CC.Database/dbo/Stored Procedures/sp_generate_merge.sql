...
IF (PARSENAME(@table_name,5)) IS NOT NULL
BEGIN
...
--making sure, not to lose any data from flot, real, money, smallmomey, datetime, date columns
...
WHEN 'ntext' THEN CHAR(10) + CHAR(9) + 'NULLIF(CAST(Source.' + @Column_Name + ' AS NVARCHAR(MAX)), CAST(Target.' + @Column_Name + ' AS NVARCHAR(MAX))) IS NOT NULL OR NULLIF(CAST(Target.' + @Column_Name + ' AS NVARCHAR(MAX)), CAST(Source.' + @Column_Name + ' AS NVARCHAR(MAX))) IS NOT NULL OR ' 
 WHEN 'geography' THEN 
 CHAR(10) + 'CASE 
       WHEN CONVERT(geography,Source.'+ @Column_Name +').IsNull IS NOT NULL AND CONVERT(geography,Source.'+@Column_Name+').STEquals(Target.'+@Column_Name+')=1 THEN NULL
       WHEN CONVERT(geography,Source.' + @Column_Name + ').IsNull IS NULL AND CONVERT(geography,Target.' + @Column_Name + ').IsNull IS NULL THEN NULL
       ELSE 1
END IS NOT NULL OR '
