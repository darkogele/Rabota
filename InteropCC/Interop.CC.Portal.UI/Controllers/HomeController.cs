...
namespace Interop.CC.Portal.UI.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult ErrorCert(ErrorModel model)
        {
            if (model.CertErrorType == CertErrorType.ExpiredCertificate)
            {
                model.ErrorText = "Внесениот сертификат е истечен. Контактирајте го администраторот";
            }
            if (model.CertErrorType == CertErrorType.InvalidCertificate)
            {
                model.ErrorText = "Внесениот сертификат е невалиден";
            }
            return View(model);
        }
        ...
        public RedirectToRouteResult RedirectToAction(string action, string controller, ErrorModel model)
        {
