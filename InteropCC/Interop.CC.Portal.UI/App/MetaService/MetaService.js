...
function ($scope, metaservicesService, interopTestCommunicationService, $state, alerts, $stateParams, providers, sharedPropertiesService) {
    ...
    $scope.interopTestCommunication = function(providerRoutingToken, providerName) {
        interopTestCommunicationService.testCommunication(providerRoutingToken)
            .then(function(response) {
                if (JSON.parse(response) == true) {
                    alerts.addSuccess('Успешна комуникација со провајдерот ' + providerName);
                } else {
                    alerts.addError('Неуспешна комуникација со провајдерот ' + providerName);
                }
            });
    };
